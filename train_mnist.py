...
class MLP(nn.Module):
    def __init__(self, input_dim, hidden_dims, output_dim):
        ...
        layers = []
        self.affine_layers = len(hidden_dims) + 1
        prev_dim = input_dim
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        output = model(data)
        loss = F.cross_entropy(output, target)  # Use Cross-Entropy loss for classification
        loss.backward()
        optimizer.step()
...
def main():
    ...
    if args.save_model:
        torch.save(model.state_dict(), "indep_mnist_mlp_{}_layers.pt".format(num_layers))
